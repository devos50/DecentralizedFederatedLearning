...
class Experiment(experimentFilePath: String) {
    ...
    private fun generateRunConfigurations() {
        configuration = RunConfiguration.defaultConfiguration(Dataset.MNIST)
        ...
        if(json.fixedValues.contains("gar")) {
    ...
    fun run() {
        ...
        evaluationProcessor.writeConfigurations(json.name, configuration, transfer)
